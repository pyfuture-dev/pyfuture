        ...
        for mode, path in changes:
            match mode:
                case Change.modified:
                    logger.info(f"Source file has been modified")
                    transfer_file(Path(path), tgt_file, target=str2tuple(target))
                case Change.deleted:
                    logger.info(f"Source file has been deleted")
                    break
...
def watch_dir(
    ...
    *,
    ...
    transfer_dir(src_dir, build_dir, target=target, target=str2tuple(target))
